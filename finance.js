...
var FINANCE_GMAIL_LEGACY_QUERY='label:"GameVault Statements" newer_than:2y';
var FINANCE_GMAIL_DEFAULT_QUERY='newer_than:2y {debited credited spent purchase paid received deposited refund reversal EMI "credit card" UPI wallet loan invoice}';
var financeGmailToken=null,financeGmailTokenClient=null,financeGmailCandidates=[],financeAutoSyncQueued=false;
...
function financeNormalize(value){
  ...
  if(!f.gmail.query||f.gmail.query===FINANCE_GMAIL_LEGACY_QUERY)f.gmail.query=FINANCE_GMAIL_DEFAULT_QUERY;
...
function financeGmailPanel(){
  ...
  return '<section class="finance-panel finance-gmail"><div class="finance-panel-head"><div><span>PRIMARY DATA SOURCE</span><h3>Gmail financial synchronization</h3></div><span class="finance-gmail-state '+(connected?'on':'')+'">'+esc(state)+'</span></div><p>GameVault searches transaction alerts from banks, cards, UPI services, wallets, lenders and shopping platforms. It separates refunds, reversals, failures, card payments and transfers before adding them to the dashboard.</p><label class="finance-gmail-query"><span>Automatic Gmail search</span><input id="financeGmailQuery" value="'+esc(g.query||FINANCE_GMAIL_DEFAULT_QUERY)+'" autocomplete="off" spellcheck="false"></label><div class="finance-actions"><button class="btn blue" data-act="finance-gmail-sync"'+(financeBusy?' disabled':'')+'>'+(financeBusy?'Checking Gmail...':(connected?'Refresh Gmail now':'Connect Gmail & scan'))+'</button><button class="btn" data-act="finance-gmail-default-query">Reset automatic search</button>'+(connected?'<button class="btn" data-act="finance-gmail-disconnect">Disconnect Gmail</button>':'')+'</div><div class="finance-gmail-meta"><span>Account: '+esc(g.email||"shown after connection")+'</span><span>Latest synchronization: '+esc(last)+'</span></div>'+(financeGmailStatus?'<div class="finance-gmail-result">'+esc(financeGmailStatus)+'</div>':'')+'<details><summary>Privacy and one-time Google setup</summary><p>Enable the Gmail API in the same Google Cloud project used by GameVault. Add the Gmail read-only scope to the OAuth consent screen and, while the app is in Testing, add your Google account as a test user.</p><p>Access tokens stay only in memory and expire quickly. Raw emails and attachments are never saved to local storage or Drive. Only approved transaction facts, EMI details and Gmail message IDs enter your encrypted Finance vault.</p></details></section>';
}
function financeStatements(){
  var pending="";
  if(financePendingImport){var emiCount=(financePendingImport.emis||[]).length;pending='<section class="finance-panel"><div class="finance-panel-head"><div><span>REVIEW BEFORE IMPORT</span><h3>'+esc(financePendingImport.name)+'</h3></div><b>'+financePendingImport.items.length+' transactions'+(emiCount?' · '+emiCount+' EMI updates':'')+'</b></div><div class="finance-import-preview">'+financePendingImport.items.slice(0,20).map(function(t){var kind=financeKind(t),positive=kind==="income"||kind==="refund"||kind==="reversal";return '<div><span>'+esc(t.date)+'</span><strong>'+esc(t.description)+'<small>'+esc(kind.replace(/_/g," "))+'</small></strong><b class="'+(positive?'income':'expense')+'">'+(positive?'+':'-')+financeMoney(t.amount)+'</b></div>';}).join("")+(emiCount?'<div class="finance-import-emi">'+emiCount+' EMI schedule update'+(emiCount===1?'':'s')+' will be merged automatically.</div>':'')+'</div><div class="finance-actions"><button class="btn blue" data-act="finance-import-confirm">Approve and update dashboard</button><button class="btn" data-act="finance-import-cancel">Cancel</button></div></section>';}
  ...
  return financeGmailPanel()+pending+'<section class="finance-panel"><div class="finance-panel-head"><div><span>SECURE IMPORT LOG</span><h3>Gmail synchronizations</h3></div></div>'+(history||'<div class="finance-empty">No Gmail transactions imported yet.</div>')+'</section>';
}
function renderFinance(){
  if(!financeUnlocked())return financeLockScreen();
  financeTouch();
  if(financeGmailConnected()&&financeState.gmail&&Date.now()-Number(financeState.gmail.lastSyncAt||0)>15*60*1000&&!financeAutoSyncQueued&&!financeBusy){financeAutoSyncQueued=true;setTimeout(function(){if(section==="finance"&&financeUnlocked()&&financeGmailConnected())financeGmailSync();else financeAutoSyncQueued=false;},400);}
...
function financeGmailReadMessage(message,token){
  ...
  return financeGmailParts(message.payload,message.id,token,files,texts).then(function(){
    ...
    var plain=financeGmailPlainText(texts);if(!items.length){var alert=financeGmailAlertTransaction(plain,meta);if(alert)items.push(alert);}
    var emi=financeGmailEmi(plain,meta,items[0]||null);
    return {id:message.id,subject:meta.subject,from:meta.from,items:items,emis:emi?[emi]:[],attachments:files.length};
  });
}
function financeGmailSync(){
  ...
  financeGmailAuthorize().then(function(t){token=t;financeGmailStatus="Searching Gmail transaction messages...";render();return Promise.all([financeGmailApi("profile",token),financeGmailListMessages(query,token)]);}).then(function(results){
    ...
    return Promise.all(messages.map(function(m){return financeGmailApi("messages/"+encodeURIComponent(m.id)+"?format=full",token).then(function(full){return financeGmailReadMessage(full,token);});})).then(function(candidates){
      financeGmailCandidates=candidates;var items=[].concat.apply([],candidates.map(function(c){return c.items;})),emis=[].concat.apply([],candidates.map(function(c){return c.emis||[];})),usable=candidates.filter(function(c){return c.items.length||(c.emis||[]).length;});
      ...
      financeGmailStatus="Checked "+candidates.length+" new message"+(candidates.length===1?"":"s")+"; "+items.length+" transaction"+(items.length===1?"":"s")+" and "+emis.length+" EMI record"+(emis.length===1?"":"s")+" detected.";
      if(items.length||emis.length)financePendingImport={id:uid(),name:"Gmail - "+usable.length+" message"+(usable.length===1?"":"s"),items:items,emis:emis,messageIds:usable.map(function(c){return c.id;}),source:"gmail"};
      return financeSave();
    });
  }).then(function(value){financeAutoSyncQueued=false;return value;}).catch(function(err){financeAutoSyncQueued=false;financeBusy=false;financeGmailStatus="";render();flash("Gmail import failed - "+(err&&err.message||"try again"));});
}
...
function financeHandleAction(act,id){
  ...
  if(act==="finance-gmail-default-query"){financeState.gmail.query=FINANCE_GMAIL_DEFAULT_QUERY;render();return true;}
