...
var FINANCE_DEVICE_DB="gamevault-finance-device-v1";
...
function financeNormalize(value){
  ...
  f.gmail.initialBackfillComplete=!!f.gmail.initialBackfillComplete;
...
function financeDeviceDb(){return new Promise(function(resolve,reject){var request=indexedDB.open(FINANCE_DEVICE_DB,1);request.onupgradeneeded=function(){if(!request.result.objectStoreNames.contains("keys"))request.result.createObjectStore("keys");};request.onsuccess=function(){resolve(request.result);};request.onerror=function(){reject(request.error||new Error("Device key storage is unavailable"));};});}
function financeDeviceKey(create){return financeDeviceDb().then(function(db){return new Promise(function(resolve,reject){var request=db.transaction("keys","readonly").objectStore("keys").get("finance-wrap");request.onsuccess=function(){if(request.result){db.close();resolve(request.result);return;}if(!create){db.close();reject(new Error("Face ID device key is missing"));return;}crypto.subtle.generateKey({name:"AES-GCM",length:256},false,["encrypt","decrypt"]).then(function(key){var write=db.transaction("keys","readwrite");write.objectStore("keys").put(key,"finance-wrap");write.oncomplete=function(){db.close();resolve(key);};write.onerror=function(){db.close();reject(write.error||new Error("Face ID device key could not be saved"));};}).catch(function(err){db.close();reject(err);});};request.onerror=function(){db.close();reject(request.error||new Error("Face ID device key could not be opened"));};});});}
function financeDeleteDeviceKey(){try{indexedDB.deleteDatabase(FINANCE_DEVICE_DB);}catch(e){}}
...
function financeGetAssertion(credentialId){return navigator.credentials.get({publicKey:{challenge:financeRandom(32),allowCredentials:[{type:"public-key",id:financeFromB64Url(credentialId)}],userVerification:"required",timeout:60000}});}
function financeEnableFace(){
  ...
  createRequest.then(function(credential){
    created=credential;var direct=financePrfOutput(credential),wrapKey=direct?crypto.subtle.importKey("raw",direct,{name:"AES-GCM"},false,["encrypt"]):financeDeviceKey(true);return Promise.all([wrapKey,crypto.subtle.exportKey("raw",financeKey),Promise.resolve(direct?"prf":"device")]);
  }).then(function(parts){
    var iv=financeRandom(12);return crypto.subtle.encrypt({name:"AES-GCM",iv:iv},parts[0],parts[1]).then(function(wrapped){
      var cfg={credentialId:financeB64Url(new Uint8Array(created.rawId)),salt:bytesToB64(prfSalt),iv:bytesToB64(iv),wrappedKey:bytesToB64(new Uint8Array(wrapped)),vaultSalt:data.finance.salt,enabledAt:Date.now(),mode:parts[2]};
      localStorage.setItem(FINANCE_FACE_STORE,JSON.stringify(cfg));financeBusy=false;financeTouch();render();flash("Face ID unlock enabled on this device"+(parts[2]==="device"?" using secure device storage":""));
    });
...
function financeUnlockFace(){
  ...
  financeBusy=true;var prfSalt=b64ToBytes(cfg.salt),unlockRequest;try{unlockRequest=cfg.mode==="device"?financeGetAssertion(cfg.credentialId).then(function(){return financeDeviceKey(false);}):financeGetPrf(cfg.credentialId,prfSalt).then(function(prf){return crypto.subtle.importKey("raw",prf,{name:"AES-GCM"},false,["decrypt"]);});}catch(err){unlockRequest=Promise.reject(err);}render();
  unlockRequest.then(function(wrapKey){
...
function financeDisableFace(){try{localStorage.removeItem(FINANCE_FACE_STORE);}catch(e){}financeDeleteDeviceKey();render();flash("Face ID unlock removed from this device");}

function financeMoney(value){return fmtMoney(Math.round((Number(value)||0)*100)/100);}
function financeCompact(value){var n=Number(value)||0,a=Math.abs(n);if(a>=1e7)return "₹"+(n/1e7).toFixed(a>=1e8?0:1).replace(/\.0$/,"")+"Cr";if(a>=1e5)return "₹"+(n/1e5).toFixed(a>=1e6?0:1).replace(/\.0$/,"")+"L";if(a>=1e3)return "₹"+(n/1e3).toFixed(a>=1e4?0:1).replace(/\.0$/,"")+"k";return "₹"+Math.round(a);}
function financeDelta(current,previous,goodWhenUp){current=Number(current)||0;previous=Number(previous)||0;if(!current&&!previous)return '<em class="fin-delta">no data last month</em>';var pct=previous?((current-previous)/previous*100):100,up=pct>=0,good=goodWhenUp?up:!up;return '<em class="fin-delta '+(good?'good':'bad')+'">'+(up?'&#9650;':'&#9660;')+' '+Math.abs(pct).toFixed(0)+'% vs last month</em>';}
...
function financeIsCardAccount(account,source){return /credit\s*card|\bcc\b|niyo|neyo|forex|card/i.test(String(account||"")+" "+String(source||""));}
function financeKind(t){
  var kind=t.kind;
  if(!kind){
    var d=String((t.description||"")+" "+(t.status||"")).toLowerCase();
    if(/failed|declined|unsuccessful|cancelled/.test(d))kind="failed";
    else if(/reversal|reversed/.test(d))kind="reversal";
    else if(/refund|refunded|cashback/.test(d))kind="refund";
    else if(t.category==="Transfer"||t.category==="Transfers"||/credit card payment|card bill payment|self transfer|own account|fund transfer|payment received|\bcred\b|billdesk|\bbbps\b|top ?up|money added|card load/.test(d))kind="transfer";
    else kind=t.type==="income"?"income":"expense";
  }
  /* Money credited onto a credit card or the Niyo forex card is the user's own
     bank transfer (bill payment / top-up), never income. Applies to stored
     kinds too, so previously imported card credits stop counting as income. */
  if(kind==="income"&&financeIsCardAccount(t.account,t.source))return "transfer";
  return kind;
}
...
function financeMonthStats(month){
  var out={month:month,credits:0,spent:0,refunds:0,netSpent:0,net:0,categories:{},merchants:{},merchantCount:{},accounts:{},count:0,ignored:0};
  (financeState.transactions||[]).forEach(function(raw){var t=financeNormalizeTransaction(raw);if(financeMonthKey(t.date)!==month)return;if(t.kind==="failed"||t.kind==="transfer"){out.ignored++;return;}if(t.kind==="income")out.credits+=t.amount;else if(t.kind==="refund"||t.kind==="reversal")out.refunds+=t.amount;else{out.spent+=t.amount;out.categories[t.category]=(out.categories[t.category]||0)+t.amount;out.merchants[t.merchant]=(out.merchants[t.merchant]||0)+t.amount;out.merchantCount[t.merchant]=(out.merchantCount[t.merchant]||0)+1;out.accounts[t.account]=(out.accounts[t.account]||0)+t.amount;}out.count++;});
...
function financeMonthlyChart(){
  var selected=financeSelectedMonth(),keys=financeMonths(12),stats=keys.map(financeMonthStats),max=Math.max.apply(null,stats.map(function(x){return Math.max(x.netSpent,x.credits);}).concat([1]));
  return '<div class="finance-comparison-chart fin-labeled" aria-label="Credits and spending by month">'+stats.map(function(s){var spent=Math.max(2,Math.round(s.netSpent/max*100)),credit=Math.max(2,Math.round(s.credits/max*100)),label=new Date(Number(s.month.slice(0,4)),Number(s.month.slice(5))-1,1).toLocaleDateString("en-GB",{month:"short"});return '<button'+(s.month===selected?' class="sel"':'')+' data-act="finance-select-month" data-id="'+s.month+'" title="'+esc(financeMonthLabel(s.month))+': spent '+esc(financeMoney(s.netSpent))+', credited '+esc(financeMoney(s.credits))+'"><span class="fin-cols"><span class="fin-col"><em class="credit">'+(s.credits?esc(financeCompact(s.credits)):'')+'</em><i class="credit" style="height:'+credit+'%"></i></span><span class="fin-col"><em class="spent">'+(s.netSpent?esc(financeCompact(s.netSpent)):'')+'</em><i class="spent" style="height:'+spent+'%"></i></span></span><b>'+label+'</b></button>';}).join("")+'</div><div class="finance-chart-key"><span><i class="credit"></i>Credited</span><span><i class="spent"></i>Spent</span></div>';
}
...
function financeTopMerchants(s){
  var rows=Object.keys(s.merchantCount||{}).map(function(k){return {merchant:k,count:s.merchantCount[k],amount:s.merchants[k]||0};}).sort(function(a,b){return b.count-a.count||b.amount-a.amount;}).slice(0,10);
  if(!rows.length)return '<div class="finance-empty">No merchant activity this month.</div>';
  var max=rows[0].count||1;
  return '<div class="fin-merchant-list">'+rows.map(function(r,i){return '<button class="fin-merchant-row" data-act="finance-open-merchant" data-id="'+esc(r.merchant)+'"><u>'+(i+1)+'</u><div><strong>'+esc(r.merchant)+'</strong><i><span style="width:'+Math.round(r.count/max*100)+'%"></span></i></div><span class="fin-merchant-count">'+r.count+'&times;</span><b>'+financeMoney(r.amount)+'</b></button>';}).join("")+'</div>';
}
function financeAccountsPanel(s){
  var rows=Object.keys(s.accounts||{}).map(function(k){return [k,s.accounts[k]];}).sort(function(a,b){return b[1]-a[1];});
  if(!rows.length)return '<div class="finance-empty">No counted spending this month.</div>';
  var max=rows[0][1]||1;
  return rows.slice(0,8).map(function(row){var card=financeIsCardAccount(row[0],"");return '<button class="finance-category-row fin-account-row" data-act="finance-open-account" data-id="'+esc(row[0])+'"><div><span>'+esc(row[0])+' <em class="fin-acct-badge">'+(card?'Card':'Bank')+'</em></span><b>'+financeMoney(row[1])+'</b></div><i><span style="width:'+Math.round(row[1]/max*100)+'%"></span></i></button>';}).join("");
}
function financeOverview(){
  ...
  return '<div class="finance-dashboard-toolbar"><div><label>Month<input id="financeOverviewMonth" type="month" value="'+esc(month)+'"></label><span>'+s.count+' records · Gmail updated '+esc(last)+'</span></div><button class="btn blue" data-act="finance-gmail-sync"'+(financeBusy?' disabled':'')+'>'+(financeBusy?'Refreshing...':'Refresh Gmail')+'</button></div>'+
    '<section class="fin-hero">'+
    '<button class="fin-kpi" data-act="finance-open-kind" data-id="income"><span>Credited</span><strong class="positive">'+financeMoney(s.credits)+'</strong>'+financeDelta(s.credits,s.previous.credits,true)+'</button>'+
    '<button class="fin-kpi" data-act="finance-open-kind" data-id="expense"><span>Spent</span><strong class="negative">'+financeMoney(s.netSpent)+'</strong>'+financeDelta(s.netSpent,s.previous.netSpent,false)+'</button>'+
    '<button class="fin-kpi" data-act="finance-open-kind" data-id="net"><span>Net cash flow</span><strong class="'+(s.net>=0?'positive':'negative')+'">'+(s.net>=0?'+':'-')+financeMoney(Math.abs(s.net))+'</strong>'+financeDelta(s.net,s.previous.net,true)+'</button>'+
    '</section><section class="finance-summary finance-summary-rich fin-chips">'+
    ...
    '<button data-act="finance-open-emi"><span>EMI this month</span><strong>'+financeMoney(s.emi)+'</strong><small>'+financeMoney(s.balance)+' balance left</small></button>'+
    ...
    '<section class="finance-panel"><div class="finance-panel-head"><div><span>MOST USED</span><h3>Top 10 merchants</h3></div><b>by visits</b></div>'+financeTopMerchants(s)+'</section>'+
    '<section class="finance-panel"><div class="finance-panel-head"><div><span>BY ACCOUNT</span><h3>Spending per card / account</h3></div></div>'+financeAccountsPanel(s)+'</section>'+
...
function financeLoans(){
  var todayKey=localISO(today()),active=(financeState.loans||[]).filter(function(x){return x.status!=="closed";});
  var totEmi=active.reduce(function(a,x){return a+(Number(x.emi)||0);},0),totBal=active.reduce(function(a,x){return a+(Number(x.balance)||0);},0);
  var counted=active.filter(function(x){return Number(x.totalInstallments)>0;});
  var paidSum=counted.reduce(function(a,x){return a+(Number(x.paidInstallments)||0);},0),totalSum=counted.reduce(function(a,x){return a+(Number(x.totalInstallments)||0);},0),leftSum=Math.max(0,totalSum-paidSum),aggPct=totalSum?Math.round(paidSum/totalSum*100):0;
  var summary=active.length?('<section class="finance-emi-summary"><div><b>'+active.length+'</b><span>Active EMI'+(active.length===1?'':'s')+'</span></div><div><b>'+financeMoney(totEmi)+'</b><span>Every month</span></div>'+(totalSum?'<div><b>'+paidSum+' of '+totalSum+'</b><span>Instalments paid</span></div><div><b>'+leftSum+'</b><span>Remaining</span></div>':'')+'<div><b>'+financeMoney(totBal)+'</b><span>Balance left</span></div></section>'+(totalSum?'<div class="finance-emi-track" role="img" aria-label="'+aggPct+'% of all instalments paid"><i style="width:'+aggPct+'%"></i><span>'+aggPct+'% of all instalments paid across every loan</span></div>':'')):'';
  var cards=financeState.loans.slice().sort(function(a,b){return (a.status==="closed")-(b.status==="closed");}).map(function(x){
    var original=Number(x.principal)||0,balance=Number(x.balance)||0,paid=Number(x.paidInstallments)||0,total=Number(x.totalInstallments)||0,pending=Number(x.pendingInstallments)||(total?Math.max(0,total-paid):0),pct=total?Math.round(paid/total*100):(original?Math.round((original-balance)/original*100):0),due=x.nextDueDate||((x.dueDay||"")+" of each month");
    var progress;
    if(total>0&&total<=36){var segs='';for(var i=0;i<total;i++)segs+='<i'+(i<paid?' class="on"':'')+'></i>';progress='<div class="finance-loan-seg" role="img" aria-label="'+paid+' of '+total+' instalments paid">'+segs+'</div>';}
    else progress='<div class="finance-loan-track"><i style="width:'+Math.max(0,Math.min(100,pct))+'%"></i></div>';
    var countLine=total?('<div class="fin-loan-count"><b>'+paid+'</b><span>of '+total+' paid</span><u>'+pending+' remaining</u></div>'):('<div class="fin-loan-count"><b>'+pct+'%</b><span>repaid</span><u>'+financeMoney(balance)+' left</u></div>');
    return '<article class="finance-loan '+(x.status==="closed"?'closed':'')+'"><div class="finance-loan-head"><div><span>'+esc(x.lender||"Detected loan")+'</span><h3>'+esc(x.name||"EMI payment")+'</h3></div><span class="finance-auto-badge">Gmail</span></div><div class="fin-loan-row">'+countLine+'<div class="fin-loan-emi"><strong>'+financeMoney(x.emi)+'</strong><small>per instalment</small></div></div>'+progress+'<div class="finance-loan-meta"><span>Balance left <b>'+financeMoney(balance)+'</b></span><span>Next due <b>'+esc(String(due||"Not detected"))+'</b></span><span>Ends <b>'+esc(x.completionDate||"Not detected")+'</b></span><span>Last paid <b>'+esc(x.lastPaymentDate||"Not detected")+'</b></span></div></article>';}).join(""),recurring=financeRecurring();
  return '<div class="finance-section-intro"><div><span>AUTOMATIC TRACKING</span><h3>EMI and recurring payments</h3><p>Built from credit-card and bank statement transactions. No manual liability entry is required.</p></div><button class="btn blue" data-act="finance-gmail-sync">Refresh statements</button></div>'+summary+'<div class="finance-loan-grid">'+(cards||'<div class="finance-empty">No EMI schedule has been detected yet.</div>')+'</div><section class="finance-panel"><div class="finance-panel-head"><div><span>RECURRING EXPENSES</span><h3>Subscriptions, bills, insurance and rent</h3></div></div><div class="finance-recurring-list">'+(recurring.length?recurring.map(function(r){var due=r.nextDate>=todayKey&&r.nextDate<=localISO(new Date(Date.now()+7*86400000));return '<article class="'+(r.increased?'warning ':'')+(due?'due':'')+'"><div><span>'+esc(r.category)+'</span><strong>'+esc(r.merchant)+'</strong><small>'+r.count+' detected payments</small></div><b>'+financeMoney(r.amount)+'</b><span>'+(r.increased?'Increased from '+financeMoney(r.average):'Expected '+esc(r.nextDate))+'</span></article>';}).join(""):'<div class="finance-empty">Recurring expenses appear after the same merchant is detected in at least two months.</div>')+'</div></section>';
}
...
function financeGmailQuery(){
  var last=Number(financeState.gmail.lastSuccessfulSyncAt||0);if(!last||!financeState.gmail.initialBackfillComplete)return FINANCE_GMAIL_DEFAULT_QUERY;
...
function financeGmailBatchSize(){return phoneUi()?6:30;}
function financeGmailConcurrency(){return phoneUi()?1:2;}
...
function financeGmailSync(automatic){
  ...
  financeGmailAuthorize().then(function(t){token=t;financeGmailStatus="Searching Gmail for attached card and bank statements...";render();return Promise.all([financeGmailApi("profile",token),financeGmailListMessages(query,token)]);}).then(function(results){
    ...
    return financeMapLimit(messages,financeGmailConcurrency(),function(m){return financeGmailApi("messages/"+encodeURIComponent(m.id)+"?format=full",token).then(function(full){return financeGmailReadMessage(full,token);});},function(done,total){financeGmailProgress={stage:"Reading statements",done:done,total:total};render();}).then(function(candidates){
...
function financeConfirmImport(){
  ...
  if(financePendingImport.source==="gmail"){
    ...
    financeState.gmail.retryMessageIds=(financePendingImport.unreadableIds||[]).slice(-100);if(!financePendingImport.hasMore)financeState.gmail.initialBackfillComplete=true;financeState.gmail.lastSuccessfulSyncAt=Date.now();financeState.gmail.lastSyncAt=Date.now();
  }
...
function financeHandleAction(act,id){
  ...
  if(act==="finance-open-account"){financeAccountFilter=id;financeCategoryFilter="";financeMerchantFilter="";financeKindFilter="";financeTab="financetransactions";render();return true;}
