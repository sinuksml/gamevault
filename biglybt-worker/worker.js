...
const WORKER_VERSION = "github-v7";
...
async function load(){if(busy||!token)return;busy=true;document.getElementById('refresh').disabled=true;setMessage('Refreshing torrents...',false);try{var fields=['id','name','status','percentDone','eta','rateDownload','rateUpload','peersConnected','peersGettingFromUs','peersSendingToUs','totalSize','sizeWhenDone','uploadRatio','queuePosition','isFinished','error','errorString'];var result=await rpc('torrent-get',{fields:fields});render(result.torrents||[])}catch(e){if(token)setMessage(e.message||'Unable to load torrents.',true)}finally{busy=false;document.getElementById('refresh').disabled=false}}
...
document.getElementById('loginForm').onsubmit=async function(e){e.preventDefault();var error=document.getElementById('loginError');error.classList.add('hidden');try{var r=await fetch('/__native/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:document.getElementById('user').value.trim(),password:document.getElementById('pass').value})}),j=await r.json().catch(function(){return {}});if(!r.ok||!j.token)throw new Error(j.message||'Login failed');saveToken(j.token);document.getElementById('pass').value='';showDashboard();load()}catch(err){showLogin(err.message||'Login failed')}};
document.getElementById('forget').onclick=function(){saveToken('');showLogin('Saved login removed from this device.')};document.getElementById('auto').onclick=function(){auto=!auto;try{localStorage.setItem(AUTO_KEY,auto?'1':'0')}catch(e){}this.textContent='Auto: '+(auto?'On':'Off')};
window.addEventListener('message',function(e){if(e.data&&e.data.type==='gvbt-native-token-response'&&!token&&typeof e.data.token==='string'&&e.data.token){saveToken(e.data.token);showDashboard();load()}});try{parent.postMessage({type:'gvbt-native-token-request'},'*')}catch(e){}if(token){showDashboard();load()}else setTimeout(function(){if(!token)showLogin('')},500);timer=setInterval(function(){if(auto&&!document.hidden)load()},15000);document.addEventListener('visibilitychange',function(){if(!document.hidden&&auto)load()});
})();
</script></body></html>`;
}
