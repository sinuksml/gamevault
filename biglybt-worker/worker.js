...
const WORKER_VERSION = "github-v13";
...
async function load(silent){if(busy||!token)return;busy=true;var refresh=document.getElementById('refresh');if(!silent){refresh.disabled=true;setMessage('Refreshing torrents...',false)}try{var fields=['id','name','status','percentDone','eta','rateDownload','rateUpload','peersConnected','peersGettingFromUs','peersSendingToUs','totalSize','sizeWhenDone','uploadRatio','queuePosition','isFinished','error','errorString'];var result=await rpc('torrent-get',{fields:fields});render(result.torrents||[])}catch(e){if(token)setMessage(e.message||'Unable to load torrents.',true)}finally{busy=false;if(!silent)refresh.disabled=false}}
