...
const WORKER_VERSION = "github-v16";
...
async function load(silent){if(busy||!token)return;busy=true;var refresh=document.getElementById('refresh');if(!silent){refresh.disabled=true;setMessage('Refreshing torrents...',false)}try{var fields=['id','name','status','percentDone','eta','rateDownload','rateUpload','peersConnected','peersGettingFromUs','peersSendingToUs','totalSize','sizeWhenDone','leftUntilDone','downloadedEver','uploadRatio','queuePosition','isFinished','error','errorString'];var result=await rpc('torrent-get',{fields:fields}),items=result.torrents||[];var completed=autoRemove?items.filter(function(t){return (t.isFinished===true||Number(t.percentDone)>=1)&&Number(t.sizeWhenDone||t.totalSize)>0}):[];if(completed.length){await rpc('torrent-stop',{ids:completed.map(function(t){return t.id})});await rpc('torrent-remove',{ids:completed.map(function(t){return t.id}),'delete-local-data':false});result=await rpc('torrent-get',{fields:fields});items=result.torrents||[];setMessage('Removed '+completed.length+' completed torrent'+(completed.length===1?'':'s')+'. Downloaded files were kept.',false)}render(items)}catch(e){if(token)setMessage(e.message||'Unable to load torrents.',true)}finally{busy=false;if(!silent)refresh.disabled=false}}
...
function armIdleTimer(){stopIdleTimer();var remaining=IDLE_MS-(Date.now()-lastActivity);if(remaining<=0){idle=true;stopPolling();updateAutoLabel();return}idleTimer=setTimeout(function(){idle=true;stopPolling();updateAutoLabel()},remaining)}
function schedulePolling(){stopPolling();if(auto&&!idle&&token&&!document.hidden)timer=setTimeout(async function(){await load(true);schedulePolling()},2000)}
async function activate(){if(!auto||idle||!token||document.hidden)return;await load();schedulePolling()}
function registerActivity(){lastActivity=Date.now();var wasIdle=idle;idle=false;updateAutoLabel();armIdleTimer();if(wasIdle)activate()}
...
document.getElementById('filters').onclick=function(e){var b=e.target.closest('button[data-filter]');if(!b)return;currentFilter=b.dataset.filter;document.querySelectorAll('#filters button').forEach(function(x){x.classList.toggle('on',x===b)});render(allItems)};
...
document.getElementById('forget').onclick=function(){stopPolling();stopIdleTimer();saveToken('');showLogin('Saved login removed from this device.')};document.getElementById('auto').onclick=function(){if(idle){registerActivity();return}auto=!auto;try{localStorage.setItem(AUTO_KEY,auto?'1':'0')}catch(e){}updateAutoLabel();if(auto)activate();else stopPolling()};document.getElementById('autoRemove').onclick=function(){autoRemove=!autoRemove;try{localStorage.setItem(AUTO_REMOVE_KEY,autoRemove?'1':'0')}catch(e){}updateAutoRemoveLabel();if(autoRemove)load(false)};
