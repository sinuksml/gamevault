...
const WORKER_VERSION = "github-v20";
...
function showDashboard(){document.getElementById('login').classList.add('hidden');document.getElementById('dashboard').classList.remove('hidden');updateAutoLabel();updateSpeedLabel()}
...
function renderHistory(){var q=document.getElementById('historySearch').value.trim().toLowerCase(),out=document.getElementById('historyOutcome').value,days=document.getElementById('historyDate').value,cutoff=days==='all'?0:Date.now()-Number(days)*86400000,items=historyItems.filter(function(h){if(q&&String(h.name||'').toLowerCase().indexOf(q)<0)return false;if(cutoff&&Number(h.at||0)<cutoff)return false;var o=String(h.outcome||'').toLowerCase();if(out==='completed'&&o.indexOf('completed')<0)return false;if(out==='removed'&&o.indexOf('removed')<0)return false;if(out==='deleted'&&!h.filesDeleted)return false;return true}),total=items.reduce(function(n,h){return n+(Number(h.downloaded)||0)},0),box=document.getElementById('historyGrid');document.getElementById('historyTotal').textContent=items.length+' records · '+bytes(total)+' downloaded';var live={};allItems.forEach(function(t){if(t.hashString)live[t.hashString]=t});box.innerHTML=items.length?items.map(function(h){return'<article class="history-card'+(h.filesDeleted?' deleted':'')+'"><div><div class="head"><div class="name">'+esc(h.name)+'</div><span class="status">'+esc(h.outcome)+'</span></div><div class="meta">'+new Date(h.at||Date.now()).toLocaleString()+' · '+bytes(h.downloaded)+' / '+bytes(h.total)+'</div></div><div class="history-actions"><b>'+(h.filesDeleted?'Files deleted':'Files kept')+'</b>'+(h.filesDeleted?'':(live[h.hash]?'<button class="btn danger" type="button" data-hdelete="'+esc(h.hash)+'">Delete torrent + files</button>':(h.hash?'<span class="hnote">Not in BiglyBT — delete on the Shield</span>':'')))+'</div></article>'}).join(''):'<div class="empty"><strong>No matching history</strong><span>Change the search, outcome or date filter.</span></div>'}
...
async function deleteHistoryFiles(hash){var entry=historyItems.find(function(h){return h.hash===hash}),t=allItems.find(function(x){return x.hashString===hash});if(!t){setMessage('This download is no longer loaded in BiglyBT, so its file cannot be deleted from here. Delete it on the Shield, or turn off "Remove completed" so finished films stay deletable.',true,true);return}var label=(entry&&entry.name)||t.name||'this torrent';if(!confirm('Permanently delete '+label+' ('+bytes(totalSize(t))+')?\\n\\nThe downloaded file will also be deleted from the drive. This cannot be undone.'))return;try{await rpc('torrent-remove',{ids:[t.id],'delete-local-data':true});knownRemoved[torrentKey(t)]=1;if(entry){entry.filesDeleted=true;entry.outcome=progress(t)>=100?'Completed - files deleted':'Deleted before completion';saveHistory();try{parent.postMessage({type:'gvbt-history-update',id:entry.id,filesDeleted:true,outcome:entry.outcome},'*')}catch(e){}}renderHistory();setMessage('Deleted '+label+' and its files.',false);await load(true)}catch(e){setMessage(e.message||'Delete failed.',true,true)}}
...
document.getElementById('forget').onclick=function(){if(!confirm('Forget the saved BiglyBT login on this device?'))return;stopPolling();stopIdleTimer();saveToken('');showLogin('Saved login removed from this device.')};document.getElementById('historyView').onclick=function(){setHistoryMode(!historyMode)};document.getElementById('auto').onclick=function(){if(idle){registerActivity();return}auto=!auto;try{localStorage.setItem(AUTO_KEY,auto?'1':'0')}catch(e){}updateAutoLabel();if(auto)activate();else{stopPolling();connection('','Auto-refresh off')}};document.getElementById('speedMode').onclick=async function(){try{altSpeed=!altSpeed;await rpc('session-set',{'alt-speed-enabled':altSpeed});updateSpeedLabel();setMessage('Alternative speed limits '+(altSpeed?'enabled.':'disabled.'),false)}catch(e){altSpeed=!altSpeed;updateSpeedLabel();setMessage(e.message||'Unable to change speed limits.',true,true)}};
document.getElementById('historySearch').oninput=renderHistory;document.getElementById('historyOutcome').onchange=renderHistory;document.getElementById('historyDate').onchange=renderHistory;document.getElementById('historyGrid').onclick=function(e){var d=e.target.closest('[data-hdelete]');if(d){deleteHistoryFiles(d.getAttribute('data-hdelete'))}};document.getElementById('historyExport').onclick=function(){var rows=[['Date','Name','Outcome','Progress','Downloaded','Total','Files deleted']].concat(historyItems.map(function(h){return[new Date(h.at||Date.now()).toISOString(),h.name,h.outcome,h.progress,h.downloaded,h.total,h.filesDeleted?'Yes':'No']})),csv=rows.map(function(r){return r.map(function(v){return'"'+String(v==null?'':v).replace(/"/g,'""')+'"'}).join(',')}).join('\\n'),a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download='gamevault-biglybt-history.csv';a.click();setTimeout(function(){URL.revokeObjectURL(a.href)},1000)};
