...
grid.onclick=async function(e){var b=e.target.closest('button[data-method]');if(!b)return;var method=b.dataset.method,id=Number(b.dataset.id),args={ids:[id]},item=allItems.filter(function(t){return Number(t.id)===id})[0];if(method==='torrent-remove'){var removeFiles=b.dataset.delete==='1';if(!confirm(removeFiles?'Remove this torrent and permanently delete its downloaded files?':'Remove this torrent from BiglyBT but keep its downloaded files?'))return;args['delete-local-data']=removeFiles}try{await rpc(method,args);if(method==='torrent-remove'&&item){knownRemoved[torrentKey(item)]=1;recordHistory(item,torrentProgress(item)>=100?'Completed - manually removed':'Manually removed before completion',b.dataset.delete==='1')}await load()}catch(err){setMessage(err.message,true)}};
