...
const WORKER_VERSION = "github-v15";
...
function showDashboard(){document.getElementById('login').classList.add('hidden');document.getElementById('dashboard').classList.remove('hidden');updateAutoLabel()}
...
document.getElementById('forget').onclick=function(){stopPolling();stopIdleTimer();saveToken('');showLogin('Saved login removed from this device.')};document.getElementById('auto').onclick=function(){if(idle){registerActivity();return}auto=!auto;try{localStorage.setItem(AUTO_KEY,auto?'1':'0')}catch(e){}updateAutoLabel();if(auto)activate();else stopPolling()};
