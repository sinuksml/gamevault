...
document.getElementById('refresh').onclick=function(){load(false)};document.getElementById('addForm').onsubmit=async function(e){e.preventDefault();var input=document.getElementById('magnet'),url=input.value.trim();if(!url)return;try{await rpc('torrent-add',{filename:url});input.value='';setMessage('Torrent added.',false);await load(true)}catch(err){setMessage(err.message,true,true)}};
