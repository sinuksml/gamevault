...
const WORKER_VERSION = "github-v6";
...
:root{color-scheme:dark;--bg:#090d14;--panel:#111927;--line:#2a3c58;--text:#eef4ff;--muted:#94a3ba;--blue:#2182ee;--danger:#e25261}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,"Segoe UI",sans-serif}.shell{max-width:1500px;margin:auto;padding:18px}.top{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:15px}.top h1{font-size:22px;margin:0 auto 0 0}.btn,input,select{border:1px solid var(--line);border-radius:8px;background:#0b121e;color:var(--text);font:inherit}.btn{padding:9px 12px;cursor:pointer;font-weight:700}.btn:hover{border-color:#5da8ff}.btn.blue{background:#176dce;border-color:#499fff}.btn.danger{color:#ffb5bc;border-color:#74343c}.btn:disabled{opacity:.45;cursor:not-allowed}.add{display:flex;gap:8px;margin-bottom:14px}.add input{flex:1;min-width:0;padding:10px 12px}.summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px;margin-bottom:14px}.stat,.card,.notice{border:1px solid var(--line);border-radius:10px;background:var(--panel)}.stat{padding:11px}.stat b{display:block;font-size:18px}.stat span{font-size:11px;color:var(--muted)}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:11px}.card{padding:13px;display:flex;flex-direction:column;gap:9px}.head{display:flex;gap:8px;align-items:flex-start}.name{font-weight:800;line-height:1.3;overflow-wrap:anywhere;flex:1}.status{font-size:10px;font-weight:800;padding:3px 7px;border:1px solid #3778bf;border-radius:999px;color:#bdddff}.meter{height:8px;background:#080d15;border:1px solid #263951;border-radius:999px;overflow:hidden}.fill{height:100%;background:linear-gradient(90deg,#2182ee,#58d4ff)}.meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;color:var(--muted);font-size:12px}.meta b{color:var(--text)}.actions{display:flex;gap:6px;flex-wrap:wrap;margin-top:auto}.actions .btn,.actions select{font-size:12px;padding:7px 9px}.notice{padding:18px;text-align:center;color:var(--muted)}.error{border-color:#8a3942;color:#ffb5bc;background:#31171c}.hidden{display:none}@media(max-width:650px){.shell{padding:10px}.summary{grid-template-columns:repeat(2,1fr)}.grid{grid-template-columns:1fr}.add{flex-wrap:wrap}.add input{flex-basis:100%}}
</style></head><body><main class="shell"><div class="top"><h1>Native BiglyBT Dashboard <small style="font-size:11px;color:var(--muted)">Optional</small></h1><button class="btn blue" id="refresh">Refresh</button><a class="btn" href="/">Web UI</a><a class="btn danger" href="/__logout">Sign out</a></div><form class="add" id="addForm"><input id="magnet" placeholder="Paste a magnet link" autocomplete="off"><button class="btn blue">Add torrent</button></form><section class="summary" id="summary"></section><div id="message" class="notice">Loading torrents...</div><section class="grid" id="grid"></section></main><script>
(function(){
var grid=document.getElementById('grid'),message=document.getElementById('message'),summary=document.getElementById('summary'),busy=false;
...
async function rpc(method,args){var r=await fetch('/__native/api',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({method:method,arguments:args||{}})});var j=await r.json().catch(function(){return {}});if(!r.ok||j.result!=='success')throw new Error(j.message||j.result||('Request failed '+r.status));return j.arguments||{}}
...
function render(items,stats){var active=items.filter(function(t){return t.status===4||t.status===6}).length,down=items.reduce(function(n,t){return n+(Number(t.rateDownload)||0)},0),up=items.reduce(function(n,t){return n+(Number(t.rateUpload)||0)},0);summary.innerHTML='<div class="stat"><b>'+items.length+'</b><span>Total torrents</span></div><div class="stat"><b>'+active+'</b><span>Active</span></div><div class="stat"><b>'+bytes(down)+'/s</b><span>Download speed</span></div><div class="stat"><b>'+bytes(up)+'/s</b><span>Upload speed</span></div>';if(!items.length){grid.innerHTML='';setMessage('No torrents found.',false);return}setMessage('',false);grid.innerHTML=items.map(function(t){var p=Math.max(0,Math.min(100,(Number(t.percentDone)||0)*100));return '<article class="card"><div class="head"><div class="name">'+esc(t.name||'Untitled torrent')+'</div><span class="status">'+esc(statusName(t.status))+'</span></div><div class="meter"><div class="fill" style="width:'+p.toFixed(1)+'%"></div></div><div class="meta"><div>Progress <b>'+p.toFixed(1)+'%</b></div><div>ETA <b>'+eta(t.eta)+'</b></div><div>Down <b>'+bytes(t.rateDownload)+'/s</b></div><div>Up <b>'+bytes(t.rateUpload)+'/s</b></div><div>Seeds <b>'+esc(t.peersGettingFromUs||0)+'</b></div><div>Peers <b>'+esc(t.peersConnected||0)+'</b></div><div>Size <b>'+bytes(t.totalSize||t.sizeWhenDone)+'</b></div><div>Ratio <b>'+Number(t.uploadRatio||0).toFixed(2)+'</b></div></div><div class="actions"><button class="btn" data-method="torrent-start" data-id="'+esc(t.id)+'">Start</button><button class="btn" data-method="torrent-stop" data-id="'+esc(t.id)+'">Pause</button><select data-priority="'+esc(t.id)+'"><option value="">Priority</option><option value="1">High</option><option value="0">Normal</option><option value="-1">Low</option></select><button class="btn danger" data-method="torrent-remove" data-id="'+esc(t.id)+'">Remove</button></div></article>'}).join('')}
async function load(){if(busy)return;busy=true;document.getElementById('refresh').disabled=true;setMessage('Refreshing torrents...',false);try{var fields=['id','name','status','percentDone','eta','rateDownload','rateUpload','peersConnected','peersGettingFromUs','peersSendingToUs','totalSize','sizeWhenDone','uploadRatio','queuePosition','isFinished','error','errorString'];var results=await Promise.all([rpc('torrent-get',{fields:fields}),rpc('session-stats',{})]);render(results[0].torrents||[],results[1])}catch(e){setMessage(e.message||'Unable to load torrents.',true)}finally{busy=false;document.getElementById('refresh').disabled=false}}
document.getElementById('refresh').onclick=load;document.getElementById('addForm').onsubmit=async function(e){e.preventDefault();var input=document.getElementById('magnet'),url=input.value.trim();if(!url)return;try{await rpc('torrent-add',{filename:url});input.value='';await load()}catch(err){setMessage(err.message,true)}};
grid.onclick=async function(e){var b=e.target.closest('button[data-method]');if(!b)return;var method=b.dataset.method,id=Number(b.dataset.id);if(method==='torrent-remove'&&!confirm('Remove this torrent from BiglyBT? Downloaded files will be kept.'))return;try{await rpc(method,{ids:[id]});await load()}catch(err){setMessage(err.message,true)}};
...
load();setInterval(function(){if(!document.hidden)load()},5000);
})();
</script></body></html>`;
}
