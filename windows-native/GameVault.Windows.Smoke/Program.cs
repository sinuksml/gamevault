...
try
{
    var contractFixture = Path.Combine(AppContext.BaseDirectory, "Fixtures", "vault-v15.json");
    ...
    Assert(contractRepository.Collection("purchasedGames").Count == 1, "Steam purchase records survive the shared contract import");
    Assert(contractRepository.Collection("purchasedGames")[0]?["permanent"]?.GetValue<bool>() == true, "purchase ownership remains permanent");
