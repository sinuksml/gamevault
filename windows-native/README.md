...
Run `installer-output\SinuGameVault-Setup-v2.6.7.exe`. The installer upgrades the existing installation under `C:\Program Files\Sinu Game Vault`, creates a Start Menu entry, optionally creates a desktop shortcut, and registers a normal Windows uninstaller.

The portable release is `SinuGameVault-Windows-v2.6.7.zip`. Extract it to a permanent directory and run `SinuGameVault.exe`.
